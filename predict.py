...
# image_path = "test/"+str(p)+".jpg"#相对路径 导入图片
# trans = transforms.Compose([transforms.Resize((120 , 120)),transforms.ToTensor()])   #将图片缩放为跟训练集图片的大小一样 方便预测，且将图片转换为张量
...
p = 3000
for i in range(1049):
    image_path = "test/"+str(p)+".jpg"  # 需要测试的图片放入当前文件夹下，这里改成自己的图片名即可
